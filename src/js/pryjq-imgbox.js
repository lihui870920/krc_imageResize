 ...
 ;(function($){
	$.extend($.fn, {
		pryImageBoxLive: function(options) {
			return $(this).livequeryPryBox(function(){
				$(this).pryImageBox(options);
			});
		}
	});
})(jQuery);
	...
	$.fn.extend({
		...
		pryImageBox: function(options) {
			...
			var defaults = {
				...
				reloadTime4reintent: 10,
				delayInit: 0,
				sumaTime4error: 150,
				sumaTime4errorMultip: 2,
				delayInit4Each: 0,
				avoidCahe: false,
				responsive: false,
			...
			return this.each(function(indxObj) {
				...
				childImage.errorClass_ = 0;
				childImage.timeWite4Error = 0;
				childImage.firstProcessed = false;
				childImage.error(function(){
					//dioError
					childImage.errorClass_ = 1;
				...
				function preLoadImage() {
					LoadImage(childImage.timeWite4Error + options.delayInit + (pryBx_indexCall * options.delayInit4Each));
					pryBx_indexCall++;
				}
				function LoadImage($timeLoadImage) {
					$timeLoadImage = $timeLoadImage + childImage.timeWite4Error;
					pryBx_dbgExecutes++;
					setTimeout(function() {
						///-------------
						...
						if (options.reloads > childImage.reloads) {
							setTimeout(function() {
								...
								childImage.timeWite4Error = (childImage.timeWite4Error + options.sumaTime4error) * options.sumaTime4errorMultip;
								LoadImage(childImage.timeWite4Error);
								childImage.reloads++;
							}, options.reloadTime4reintent);
						} else {
							if (options.autoOcult_OnErrorLoad) {
								//dioError
								childImage.errorClass_ = 3;
								childImage.error_ = true;
								sprThis.css('visibility', 'hidden');
								childImage.hasRun = true;
							}
